...
export function SqueezeCarousel({
    ...
    const wrap = (i: number) => count ? ((i % count) + count) % count : 0;
        ...
        >
            ...
            {controls && count > 1 && (
                <div className="mb-4 flex justify-end gap-2">
                    <Arrow back label="สไลด์ก่อนหน้า" onClick={() => step(-1)} />
                    <Arrow label="สไลด์ถัดไป" onClick={() => step(1)} />
                </div>
            )}
                    ...
                    {cards.map((card, place) => {
                        ...
                        return (
                            <button
                                ...
                                className={cn(
                                    "relative isolate h-full shrink-0 cursor-pointer overflow-hidden border-0 bg-muted p-0",
            ...
            <div id={`${ids}-panel`} role="tabpanel" aria-live="polite" className="mt-6 grid @xl:mt-7">
                {slides.map((slide, i) => {
                    ...
                    return (
                        ...
                        >
                            <p className="m-0 max-w-[46rem] text-[15px] leading-[1.6] text-balance @lg:text-[17px]">
...
function Arrow({
    ...
    return (
        <button
            ...
            onClick={onClick}
            className={cn(
                "grid size-9 cursor-pointer place-items-center rounded-md border-0",
...
function Action({ slide, shown }: { slide: SqueezeSlide; shown: boolean }) {
    ...
    const dress = cn(
        "group/sq-action inline-flex shrink-0 cursor-pointer items-center gap-2 rounded-md border-0",
